...
Litch's Cortex 是一个对话资产治理工具，用于管理 Litch 与多个 AI 进行深度对话产生的 PDF 和对话记录。核心数据流为：**创建项目 → 上传 PDF / 导入对话 JSON → 解析分段 → LLM 提取话题标签 → 查看话题下的原文 → 按话题合并分段 → 生成总结**。

V0.8 的核心变更是 **ChatGPT 对话 JSON 导入**：支持导入 ChatGPT `conversations.json` 文件，实现对话资产与 PDF 资产并行纳入话题提取和语义搜索 pipeline。实现三级去重（对话级 → 消息级 → Chunk 级），流式解析大文件，完整的导入归因日志。
...
chunks: id, documentId(?), conversationId(?), content, position, tokenCount, stableId(?), createdAt  ← V0.8 扩展
...
conversations: id, projectId, cortexUserId, externalId, title, source, model, messageCount, createTime, updateTime, status, rawMetadata, importLogId, createdAt, updatedAt  ← V0.8 新增
conversationMessages: id, conversationId, externalMessageId, role, content, contentHash, position, modelSlug, createTime, createdAt  ← V0.8 新增
importLogs: id, cortexUserId, projectId, filename, fileSize, conversationsTotal/Imported/Skipped/Updated, messagesTotal, chunksCreated/Skipped, conflicts, errors, status, startedAt, completedAt, durationMs  ← V0.8 新增
...
## ChatGPT 对话导入架构（V0.8 核心变更）

### 数据流

```
conversations.json → ChatGPT Parser → Conversation Chunker → DB (三级去重)
                     (纯函数)          (Q&A 对分段)           (stableId 幂等)
```

### 模块架构

```
server/chatgpt-parser.ts          → 纯函数 ChatGPT JSON 解析器（0 DB 依赖）
├── extractMainChain(conv)         → current_node 回溯主链
├── isVisibleMessage(node)         → 7 条过滤规则
├── extractMessageContent(msg)     → 拼接 content.parts 字符串
├── computeContentHash(content)    → SHA-256 hex
├── parseConversation(raw)         → 编排所有步骤 → ParsedConversation
└── isChatGPTConversation(obj)     → 类型守卫

server/conversation-chunker.ts     → Q&A 对分段器
├── buildStableId(convId, msgId, chunkIdx, content)  → 确定性 stableId
├── groupIntoQAPairs(messages)     → 配对连续 user+assistant
├── formatQAPair(pair)             → [Q]...[A]... 格式
└── chunkConversation(convId, title, messages)  → ConversationChunk[]

server/import-service.ts           → 导入编排服务
├── importConversationsJson(params)  → 创建 import_log → 后台异步执行
├── getImportProgress(importLogId)   → 内存中进度查询
└── 三级去重：对话级(externalId) → 消息级(contentHash) → chunk级(stableId)
```

### 7 条默认规则（来自 Root 外部评审）

1. **主链 = current_node 回溯**（不是最长分支）
2. **只索引可见 user/assistant 消息**
3. **跳过 system/tool/hidden/user_editable_context/weight=0**
4. **conversation_id + message.id = 稳定源 ID**
5. **stableId = chatgpt:{conv_id}:{msg_id}:{chunk_idx}:{hash8}**
6. **增量更新按 message_id + contentHash diff**
7. **原始 JSON 作审计层（rawMetadata），不进搜索索引**

### 分段策略

- Q&A 对为自然单元：连续 user + assistant 消息配对
- 每对加前缀 `[对话: {title} | Turn N]`
- ≤ 800 字符 → 单 chunk；> 800 字符 → 按句子边界拆分（复用 chunkText）
- 孤立 user/assistant 消息单独成 chunk

### 流式解析

- < 10MB → 直接 `JSON.parse()`
- ≥ 10MB → `stream-json` StreamArray 流式解析
- 每批 50 条对话处理

### 导入进度

- 内存 `Map<importLogId, ImportProgress>` 存储实时进度
- tRPC `conversation.importProgress` 轮询（2s）
- 完成后 5 分钟清理内存，之后 fallback 到 DB `import_logs` 表
...
### 3. 项目工作区 (`/project/:id`)
- 侧边栏显示项目信息和导航菜单
- 包含子页面：上传文档、分段预览、话题列表、话题探索、**对话导入（V0.8）**
...
### 9. 对话导入 (`/project/:id/conversations`)（V0.8 新增）
- 拖拽或点击上传 ChatGPT `conversations.json` 文件
- 实时导入进度（进度条 + 统计卡片：导入/跳过/更新/chunks）
- 已导入对话列表（分页，显示标题/消息数/模型/时间/状态）
- 导入历史记录（文件名/统计/耗时/状态）
- 支持增量导入：重复对话自动跳过，更新的对话自动 diff
...
### 11. 设置页 (`/settings`)（V0.5 新增，V0.6 增强）
...
### 12. Prompt 模板管理 (`/settings/templates`)（V0.5 新增）
- 列表展示所有模板（预设 + 自定义）
...
```
drizzle/schema.ts                      → 数据库表定义（V0.8: +conversations/conversationMessages/importLogs 表、chunks 扩展）
server/chatgpt-parser.ts              → V0.8 ChatGPT JSON 解析器（纯函数，主链回溯+过滤+hash）
server/conversation-chunker.ts        → V0.8 对话分段器（Q&A 对+stableId）
server/import-service.ts              → V0.8 导入编排服务（流式解析+三级去重+归因日志）
server/conversationUploadRoute.ts     → V0.8 对话 JSON 上传 Express 路由
...
server/db.ts                           → 数据库查询层（V0.8: +conversation/message/importLog CRUD、V2 embedding 查询）
server/routers.ts                      → tRPC 路由（V0.8: +conversation router 6 端点）
...
client/src/pages/ProjectWorkspace.tsx  → 项目工作区容器（V0.8: +对话导入侧边栏项）
client/src/pages/Conversations.tsx    → V0.8 对话导入页（上传+进度+列表+历史）
...
server/v08.test.ts                     → V0.8 对话导入测试（53 个测试：parser+chunker+stableId+router）
server/__fixtures__/chatgpt-sample.json → V0.8 测试 fixture（3 条测试对话）
...
| `conversation.list` | query | **V0.8** 分页获取项目对话列表 |
| `conversation.get` | query | **V0.8** 单个对话详情 |
| `conversation.messages` | query | **V0.8** 对话消息列表 |
| `conversation.chunks` | query | **V0.8** 对话产生的 chunks |
| `conversation.importProgress` | query | **V0.8** 导入进度（内存+DB fallback） |
| `conversation.importHistory` | query | **V0.8** 导入历史记录 |
| `POST /api/upload/conversations` | REST | **V0.8** 对话 JSON 上传（multipart/form-data，500MB 限制） |
...
29. **V0.8** 对话导入通过 `POST /api/upload/conversations` 上传 JSON 文件，后台异步执行
30. **V0.8** `chatgpt-parser.ts` 是纯函数模块（0 DB 依赖），`import-service.ts` 编排三级去重
31. **V0.8** chunks 表现在同时服务 PDF 和对话来源，通过 `documentId`/`conversationId` 区分（互斥可空）
32. **V0.8** `stableId` 字段实现 chunk 级幂等去重，格式：`chatgpt:{conv_id}:{msg_id}:{chunk_idx}:{hash8}`
